...
// View at https://supabase.com/dashboard/project/lmffeqwhrnbsbhdztwyv/database/tables
...
export const mget = async (keys: string[]): Promise<any[]> => {
  const supabase = client()
  const { data, error } = await supabase.from("kv_store_6fcaeea3").select("value").in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
...
export const getByPrefix = async (prefix: string): Promise<any[]> => {
  const supabase = client()
  const { data, error } = await supabase.from("kv_store_6fcaeea3").select("key, value").like("key", prefix + "%");
  if (error) {
    throw new Error(error.message);
  }
  return data?.map((d) => d.value) ?? [];
